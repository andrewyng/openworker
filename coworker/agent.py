...
def build_engine(
    *,
    ...
    def context_provider() -> str:
        ...
        # Date and git state, recomputed per turn (HEAD). Deliberately AFTER the live
        # clock so the clock stays per-turn, and not in the system prompt: this is the
        # text that changes every time the agent writes a file or commits, and in the
        # prefix it invalidated the cache for everything behind it. See
