...
class TurnEngine:
    ...
    async def run(
        ...
        *,
        ...
        self._begin_turn()
        # session_facts.begin_turn() — origin/main: record the turn at the session level.
    ...
    def _append_notice(self, kind: str, text: Optional[str] = None, **fields: Any) -> None:
        ...
        fire, and two stacked markers leave a non-resumable one on the end. Extra `fields`
        (e.g. the failing MCP server's name) persist on the message for structured rendering."""
        # At most one closing marker per turn (HEAD): if we're already closed, a second
        # marker would just be noise in the transcript.
