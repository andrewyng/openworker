...
class PermissionEngine:
    ...
    # Commands that ALWAYS require the user, in every mode and past every allowlist (HEAD).
    # Matched per segment against the whole command; see `_gated`.
    gated_commands: list[str] = field(default_factory=list)
    # Egress domains that auto-run without a prompt (origin/main): `allowed_domains` from user
    # config, plus `session_allow_domains` minted by "Always allow this domain". Matched by
    # exact host or subdomain suffix (see `_domain_allowed`).
    ...
    def evaluate(
        ...
        # Gated commands always ask (HEAD). Deliberately ahead of the self-protection floor,
        # AUTO mode, the command allowlist, the session allowlist and custom-mode auto-allow:
        # these are the irreversible acts (publishing, deploying, force-resetting), everything
        # else a run does is recoverable from git, and no permission accumulated during a run
        # should be able to widen quietly into them. Default is empty — a machine only gets
        # this by asking for it in config.
        if is_shell:
        ...
        # A protected in-project target (git hooks, CI config) (origin/main) skips every
        # auto-approve path below — including auto mode and the session/config allowlists — and
        # asks.
    ...
    def _gated(self, command: str) -> str:
        """The gate pattern this command trips, or "" if none (HEAD).
    ...
    # The self-protection floor + egress gate (origin/main), added as part of the merge.
