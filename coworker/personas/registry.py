...
class PersonaEntry:
    ...
    tagline: str = ""
    builtin: bool = True
    # HEAD: the persona's family and workspace requirement, surfaced to the GUI so it
    # can detect project-scoped personas (git/project) uniformly. Manifest-backed
    # personas carry these verbatim; builtins set them at registration.
    ...
    projects: bool = True
    # origin/main: workspace/toolset traits (workspace-scratch-design.md). requires_folder is
    # the composer/engine gate on a user-picked primary folder — surfaced to the GUI, which
...
class PersonaRegistry:
    ...
    def _register_builder(
        ...
        group="general",
        # Legacy family/workspace pair (still emitted for older sidecars; the GUI
        # prefers `requires_folder` when present). Main's refactor collapsed this into
        # requires_folder/subagents/scheduling — both coexist here by design.
        family="knowledge",
    ...
    def _register_manifest(self, m, *, builtin: bool) -> None:
        self._entries[m.id] = PersonaEntry(
            ...
            tagline=m.tagline,
            builtin=builtin,
            # Manifest no longer carries family/workspace (workspace-scratch-design.md);
            # default them. Manifest-backed personas declare their real surface via
            # requires_folder/subagents/scheduling below.
            family="knowledge",
            workspace="deliverable",
