...
function ConnectInline({
  ...
  return (
    <div>
      ...
      >
        <Icon name="arrowLeft" size={13} /> {t("Connect {{connector}}", { connector: c.title })}
      </button>
      ...
      <p className="text-[10.5px] text-faint mt-2 leading-snug">
        {t("Connecting makes {{connector}} available to all your coworkers — the toggle in this list controls just this session.", { connector: c.title })}
      </p>
    </div>
  );
}
...
function ChannelsInline({
  ...
  return (
    <div>
      ...
      {channels.length === 0 ? (
        <div className="text-[12px] text-faint py-0.5">
          {t("Not listening to any {{connector}} channel yet.", { connector: label })}
        </div>
      ) : (
        <div className="space-y-1">
          {channels.map((s) => (
