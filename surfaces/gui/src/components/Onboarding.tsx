...
const TOOL_ROWS = [
  { name: "outlook", benefit: "掌控邮件", detail: "Outlook — 整理邮件、起草回复、管理日程。" },
  { name: "slack", benefit: "跟进 Slack", detail: "Slack — 查看动态、回复提及、发布更新。" },
  { name: "github", benefit: "交付代码", detail: "GitHub — 审查 PR、关注 issue、回复 @提及。" },
  { name: "notion", benefit: "随时查阅笔记", detail: "Notion — 搜索页面、查询数据库、起草文档。" },
  { name: "hubspot", benefit: "保持 CRM 最新", detail: "HubSpot — 更新商机、记录备注、准备通话。" },
  { name: "attio", benefit: "追踪每段关系", detail: "Attio — 搜索记录、查看时间线、记录备注。" },
];
      ...
      <div className="w-[600px] max-w-[92vw] h-[560px] max-h-[88vh] rounded-2xl border border-line bg-panel shadow-2xl p-8 flex flex-col">
        ...
        {step === 1 && (
          ...
          <section data-testid="ob-step-tools" className="flex-1 min-h-0 flex flex-col">
            <h1 className="text-[19px] font-semibold">连接您的常用工具</h1>
            <p className="text-[13px] text-muted mt-0.5 mb-3">
              仅对话只能提供建议。连接后，您的协作伙伴才能完成实际工作：
            </p>
                  ...
                  >
                    ...
                    {cloud?.signed_in &&
                      (c.connected ? (
                        ...
                        <span className="text-[12px] text-muted shrink-0">请查看您的浏览器…</span>
                      ) : (
                        <button
            ...
            {!cloud?.signed_in ? (
              ...
              >
                <span className="block text-[13px] font-semibold text-ok mb-0.5">
                  🎉 您已登录{cloud.account ? `，账户为 ${cloud.account}` : ""}
                </span>
                <span className="block text-[12.5px] text-muted">
                  一键连接上方的工具 - 或稍后随时从连接器页面添加。
                </span>
              </div>
            )}
            ...
            <p className="text-[11px] text-faint mt-3">
              连接器页面还有 30+ 个工具 - 随时添加或移除。令牌保留在这台 Mac 上。
            </p>
          </section>
        )}
        ...
        {step === 2 && (
          <section data-testid="ob-step-done" className="flex-1 min-h-0 flex flex-col overflow-y-auto">
            <div className="text-center">
              ...
              <h1 className="text-[19px] font-semibold mb-1">您已完成设置</h1>
              <p className="text-[13px] text-muted mb-5">两种不错的开始方式：</p>
            </div>
            ...
            >
              ...
              <span className="flex-1 min-w-0 text-left">
                <b className="block text-[13.5px]">创建您的第一个自动化</b>
                <span className="text-[12px] text-muted">
                  每周摘要、晨间简报 - 选择模板，两分钟即可运行。
                </span>
              </span>
            ...
            >
              ...
              <span className="flex-1 min-w-0 text-left">
                <b className="block text-[13.5px]">开始使用 Coworker</b>
                <span className="text-[12px] text-muted">
