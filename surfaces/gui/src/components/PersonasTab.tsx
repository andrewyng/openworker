...
export function PersonasTab({ onOpenPersona }: { onOpenPersona?: (id: string) => void }) {
  ...
  const install = async () => {
    ...
    if (r.personas) setPersonas(r.personas);
    setMsg(
      t("Installed {{count}} persona(s) — review and enable below.", {
            ...
            {confirmOff === p.id && (
              ...
              >
                <span className="min-w-0">
                  {t("Disabling archives its {{count}} conversation{{plural}} — they stay available under “Show archived”.", {
                    count: liveCount(p.id),
                    plural: liveCount(p.id) === 1 ? "" : "s",
                  })}
                </span>
                <button
          ...
          {consent.map((c) => (
            <div key={c.id} className={CARD + " p-3.5"}>
              ...
              <div className="text-[12px] text-ink">
                {t("Tools: {{value}}", { value: c.tools.join(", ") || "—" })}
              </div>
              <div className="text-[12px] text-ink">
                {t("Risk: {{value}}", { value: c.risk.join(", ") || t("read") })}
                {c.connectors ? t(" · connectors") : ""}
                {c.messaging ? t(" · messaging") : ""}
                {c.mcp.length ? t(" · mcp: {{value}}", { value: c.mcp.join(", ") }) : ""}
              </div>
