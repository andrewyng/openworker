...
const PERMISSION_OPTIONS: Option[] = [
  { value: "discuss", label: "讨论", description: "聊天与探索 - 不执行编辑或命令" },
  { value: "interactive", label: "请求审批", description: "在执行编辑和命令前询问" },
  { value: "auto", label: "完全访问", description: "无需询问运行所有操作" },
];
  ...
  const addFiles = async (files: FileList | File[]) => {
    ...
    for (const file of list) {
      if (isPdfFile(file) && file.size > maxMb * 1024 * 1024) {
        showAttachNotice(
          `${file.name} 已跳过 — ${(file.size / 1024 / 1024).toFixed(1)} MB 超过了您的 ${maxMb} MB 限制（设置 → Token 节省）`,
        );
        continue;
      }
      accepted.push(file);
    ...
    for (const a of read) {
      if (a.kind === "pdf" && a.data_url) {
        ...
        if (info?.ok && (info.pages ?? 0) > maxPages) {
          showAttachNotice(
            `${a.name} 已跳过 — ${info.pages} 页超过了您的 ${maxPages} 页限制（设置 → Token 节省）`,
          );
          continue;
        }
        if (info && !info.ok) {
  ...
  const toggleDictation = async () => {
    ...
    try {
      if (dictation?.recording) {
        ...
        if (transcript === null) throw new Error("无法转写您的录音。");
        if (transcript.trim()) {
  ...
  return (
    <div className="relative">
      <button
        ...
        title={
          showBar
            ? `上下文窗口已使用 ${pct}% · 本次会话 ${formatTokens(total)} 个 token`
            : `本次会话的 Token 用量：${formatTokens(total)}`
        }
          ...
          >
            {contextWindow ? (
              ...
              <div className="mb-2.5 text-[11.5px] text-muted tabular-nums">
                当前上下文：{formatTokens(usage.context)} 个 token
              </div>
            ) : null}
            <div className="text-[10.5px] uppercase tracking-[0.06em] text-faint font-semibold mb-1">
              会话总计
                  ...
                  <div className="mt-0.5 flex flex-col gap-0.5">
                    {t.cache_read + t.cache_write > 0 ? (
                      <>
                        ...
                        {stat("输入总计", t.input + t.cache_read + t.cache_write)}
                      </>
                    ) : (
                      stat("输入", t.input)
                    )}
            ...
            <div className="mt-2 pt-2 border-t border-line flex items-baseline justify-between text-[11.5px]">
              <span className="text-faint">总计</span>
              <span className="text-ink tabular-nums">{formatTokens(total)} 个 token</span>
            </div>
              ...
              <>
                ...
                <div className="flex items-center gap-2 px-2.5 py-1.5">
                  <span className="flex-1 min-w-0">
                    <span className="block text-[13px] text-ink">将审批发送至收件箱</span>
                    <span className="block text-[11px] text-faint leading-snug">
                      审批与问题将发送至收件箱；智能体继续工作。
                    </span>
                  </span>
                  <Toggle
                    checked={!!unattended}
