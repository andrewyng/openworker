  ...
  return (
    <div className="px-5 py-4 space-y-3">
      <p className="text-[13px] text-muted">
        {t("Opens {{title}} in your browser — sign in and approve access there. No tokens typed, and no OpenWorker account needed: the sign-in runs entirely on this computer.", { title: c.title })}
      </p>
      <button
        ...
        disabled={waiting}
      >
        {waiting ? t("Check your browser…") : t("Connect {{title}}", { title: c.title })}
      </button>
      {error && <div className="text-[12.5px] text-danger">{error}</div>}
      <p className="text-[12px] text-faint text-center flex items-center justify-center gap-1.5">
        <span className={TAG_ACCENT}>{t("Recommended")}</span> {t("agents get a curated set of {{title}} tools · tokens stay on this computer", { title: c.title })}
      </p>
    </div>
  );
}
...
function GenericOneClick({ c, cloud }: { c: Connector; cloud: CloudStatus | null }) {
  ...
  return (
    <div className="px-5 py-4 space-y-3">
      <p className="text-[13px] text-muted">
        {t("Opens {{title}} in your browser — approve access there. No tokens typed; connect again with another account to add it alongside.", { title: c.title })}
      </p>
      {cloud?.signed_in ? (
        <button
          className={PILL_ACCENT + " w-full !py-2"}
  ...
  return (
    <div className="px-5 py-4 space-y-3">
      ...
      <div className="space-y-1.5" data-testid="hubspot-access">
        {(
          [
            ["read", t("Read-only"), t("search and read contacts, companies, deals, tickets")],
            ["write", t("Read & write"), t("adds: log notes and tasks, update records, create contacts — never delete")],
          ] as const
