    ...
    <Shell>
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <PanelHead title={tr("Automations")} sub={tr("Recurring tasks OpenWorker runs on a schedule.")} />
        </div>
        <button
      ...
      <div className="text-[12px] text-faint flex gap-1.5 mb-4">
        ...
        <span>
          {tr("Runs only while openworker-server is up — a missed schedule catches up once when it next starts.")}
        </span>
      </div>
      ...
      {empty ? (
        !showForm && (
          <div className={CARD + " p-4 text-[12.5px] text-muted"}>
            ...
            {tr(", or just ask OpenWorker in a session.")}
          </div>
        )
      ) : (
