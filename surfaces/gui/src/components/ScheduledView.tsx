...
const fmt = (timestamp: number | null, language: "en" | "zh-CN") =>
  timestamp
    ? new Date(timestamp * 1000).toLocaleString(language === "zh-CN" ? "zh-CN" : "en-US", {
        dateStyle: "medium",
        timeStyle: "short",
      })
    : "—";
      ...
      {empty ? (
        !showForm && (
          <div className={CARD + " p-4 text-[12.5px] text-muted"}>
            {t("No scheduled tasks yet — use a template above, create a new automation, or ask OpenWorker in a session.")}
          </div>
        )
      ) : (
          ...
          {tasks.map((task) => (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 text-[12px] text-muted">
                ...
                {task.enabled ? task.schedule : t("Paused")} · {t("next")} {fmt(task.next_run, language)} · {t("{{count}} runs", { count: task.run_count })}
                {task.last_status ? ` · ${t("last")} ${t(task.last_status)}` : ""}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="sched-detail">
        ...
        {editing ? (
          ...
          <div className="conn-meta">
            ...
            {task.enabled ? t("Active · next {{time}}", { time: fmt(task.next_run, language) }) : t("Paused")} · {task.schedule}
          </div>
        )}
          ...
          >
            <div className="sched-run-row">
              <span>
                ...
                {fmt(r.started_at, language)} · <span className={"run-" + r.status}>{t(r.status)}</span> · {t(r.trigger)}
                {r.artifacts.length > 0 && <span className="dim"> · {t("{{count}} files", { count: r.artifacts.length })}</span>}
              </span>
