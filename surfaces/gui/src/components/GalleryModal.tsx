  ...
  const detailView = detailSlug && (
    <div data-testid="gallery-detail">
      ...
      {!detail ? (
        ...
        <div className="text-[12.5px] text-danger">{detail.error || t("gallery.could_not_load")}</div>
      ) : (
        ...
        <div className="overflow-y-auto hairline-scroll p-5">
          {loading ? (
            <div className="space-y-2" data-testid="gallery-loading" aria-busy="true">
              <div className="text-[12.5px] text-muted mb-3">{t("gallery.loading_gallery")}</div>
              {[0, 1, 2].map((i) => (
