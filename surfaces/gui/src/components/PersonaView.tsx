      ...
      <div className="flex-1 overflow-y-auto hairline-scroll">
        <div className="max-w-3xl mx-auto px-7 py-6 space-y-6">
          ...
          {detail.tools.length > 0 && (
            <section>
              <div className={`${SEC_H} mb-2`}>{t("persona.builtin_capabilities")}</div>
          ...
          {detail.recommends.length > 0 && (
            <section>
              ...
              <p className="text-[12.5px] text-muted mb-2.5">
                {t("persona.connections_desc", { name: shortPersonaName(detail.name, personaId) })}
              </p>
          ...
          {detail.default_connections.length > 0 && (
            <section>
              ...
              <p className="text-[12.5px] text-muted mb-2.5">
                {t("persona.new_sessions_desc", { name: shortPersonaName(detail.name, personaId) })}
              </p>
                ...
                {detail.default_connections.map((c) => (
                  ...
                  >
                    ...
                    <div className="flex-1 text-[13px] font-medium">
                      ...
                      {!c.connected && (
                        <span className="text-[11px] text-faint font-normal"> · {t("persona.connect_to_enable")}</span>
                      )}
                    </div>
                    <Toggle
                      checked={c.enabled}
          ...
          <section className="flex flex-wrap gap-x-8 gap-y-2 text-[12.5px]">
            ...
            {detail.workspace && (
              <div>
                <span className="text-faint">{t("persona.workspace_label")}</span> · {detail.workspace}
              </div>
            )}
          </section>
        </div>
