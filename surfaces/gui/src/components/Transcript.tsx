  ...
  return (
    <div className="relative h-0 select-none">
      ...
      >
        ...
        {when && (
          <span data-testid="bubble-ts" title={when.toLocaleString(language)}>
            {when.toLocaleTimeString(language, { hour: "numeric", minute: "2-digit" })}
          </span>
        )}
      </div>
    </div>
...
function ApprovalChip({ resolved }: { resolved: ApprovalDecision | undefined }) {
  ...
  return (
    <span
      ...
      title={resolved ? `${t("approved")} · ${resolved.replace(/_/g, " ")}` : t("approved")}
    >
      ✓ {t("approved")}
    </span>
  );
