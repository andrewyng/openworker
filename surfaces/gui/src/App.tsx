import { useCallback, useEffect, useMemo, useLayoutEffect, useRef, useState, type PointerEvent } from "react";
...
export function App() {
  ...
  const saveAsProject = async () => {
    ...
    if (!dest.path) return;
    const res = await saveSessionAsProject(sessionId, dest.path);
    if (!res.ok || !res.path) {
      setItems((p) => [
        ...p,
  ...
  // §22 subtitle: the facts row above the composer. Persona shows its full name here
  // (e.g. "Security Coworker"), matching the line the session header has always carried; the
  // short label ("Security") is reserved for the composer's persona chip (see <Composer
  // personaLabel>). main asserts the full name in family-gate:32.
  const subtitleParts = [fullPersonaName(personaOf(agent)?.name, agent), modelDisplay];
  if (isProjectScoped(activePersona) && workspace) {
  ...
  const showSaveAsProject = hasHistory && tempWorkspace && isProjectScoped(activePersona);
