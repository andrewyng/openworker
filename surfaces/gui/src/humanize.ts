...
function humanizeConnectedTool(
  name: string,
  a: Record<string, any>,
): HumanLine | null {
  switch (name) {
...
export function humanizeTool(name: string, args: any): HumanLine {
  ...
  const known =
    humanizeShellTool(name, a) ||
    humanizeFileTool(name, a) ||
    humanizePlanTool(name, a) ||
    humanizeConnectedTool(name, a);
