...
// Fixture state: browser + slack + github connected/enabled. Browser Use is a built-in
// capability and therefore does not appear as a source; github is two_way WITHOUT
// channels — relay mentions, no subscriptions), gmail recommended-not-connected, one
// primary root → summary "Slack, GitHub · 1 folder".
...
test("per-session mute round-trips; the summary follows", async ({ page }) => {
  ...
  await body.getByTitle("Enabled for this session — tap to mute here").first().click();
