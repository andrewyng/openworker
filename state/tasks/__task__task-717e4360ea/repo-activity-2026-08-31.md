# Repo activity brief — 2026-08-31 (window: last 24 hours)

**Headline — access regression, fourth consecutive run:** the GitHub MCP gateway
...
same failure mode as the **2026-08-27** and **2026-08-28** runs (gateway returns
no response at all) and is distinct from the earlier `Authentication Failed: Bad
credentials` on **2026-08-24** (a reachable gateway, bad token). **Consequence:
0 of 10 repos verifiable today.** No commit data was obtainable, so nothing in
the last 24h can be reported from primary source — all results are "unknown", not
"no activity". Shell path is unchanged (no `GITHUB_TOKEN`, no `gh` auth), so
there is no fallback to primary source on this run.
...
They are carried, not rediscovered.
...
## ADJACENT (carried forward, API-unverified this run)

- **liaison-agentSystem [ADJACENT]** — **stalled.** First flagged 2026-08-20;
  last commit was already **2026-08-17** (a bare "WIP snapshot" of an EVO-X2
  migration) as of the 08-24 brief. As of today ~14 days quiet — firmly stalled.
  **Cannot re-confirm** via API today (gateway down); carried unchanged.
- **Sigma (ragtradesystem / sigma branch) — pre-open market brief** — automation
  disabled; last run **2026-08-15**. **Unverified via API** today; unchanged.
...
outage, now for the **fourth consecutive run** (08-27, 08-28, 08-31 identical,
08-24 a different error but same 9/10-blind outcome). No per-repo budget
truncation occurred; this is a hard access failure, so the "12-call budget" was
not a constraint here — the gateway refused every call. No new commits can be
reported until the gateway is reachable (or a token/shell path works — shell still
has no `GITHUB_TOKEN`/`gh` auth). This is a gateway/credential problem, not a
research finding.

**Net:** nothing verifiable this run — same as 08-27 and 08-28. Last successfully
verifiable brief: **2026-08-28** was likewise all-unverifiable; the last truly
verifiable one was **2026-08-24** (liaison-agentSystem only, via the then-reachable
but mistokened gateway), with the on-disk state of agpack / dcode-stack /
workstation-stack last recorded in FOCUS.md. Prior state carried forward verbatim.
